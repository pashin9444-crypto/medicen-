...
function defaults() {
  return {
    ...
    texts: {},   // free-form text/color/font edits, keyed by page + element
    updatedAt: null,
    updatedBy: null,
  };
}
...
async function saveContent(input, byEmail) {
  ...
  if (input.texts && typeof input.texts === "object") {
    ...
    Object.keys(input.texts).slice(0, 2000).forEach(function (k) {
      ...
      if (t.text != null) clean.text = String(t.text).slice(0, 4000);
      if (t.color != null && /^(#[0-9a-fA-F]{3,8}|[a-zA-Z]+)$/.test(String(t.color))) clean.color = String(t.color);
