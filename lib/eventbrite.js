...
async function syncEvents(events) {
  ...
  for (const e of events) {
    ...
    try {
      if (e.eventbriteId) {
        await api("/events/" + encodeURIComponent(e.eventbriteId) + "/", "POST", payload);
        // update the picture only if it changed since last sync
        if (e.image && e.ebImage !== e.image) {
          try { await setLogo(e.eventbriteId, e.image); out.push(Object.assign({}, e, { ebImage: e.image })); changed = true; }
          catch (le) { console.error("Eventbrite logo update failed for", e.id, "-", le.message); out.push(e); }
        } else { out.push(e); }
      } else {
